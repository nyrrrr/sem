...
public class Application extends Controller {
	 ...
	 */
	public ArrayList<JsonNode> getLocalEvents(String longitude, String latitude, int radius, boolean festivalsOnly) {
	...
	/**
	 * Send artist.getEvents request to Last.fm, and send SPARQL query to get artist information.
	 * Parse all responses and store the information in the respective objects - Artist, Event, Venue.
	 ...
	 */
	public JsonNode getArtistEvents(String artist, boolean festivalsOnly) {		
	...
	/**
	 * Send venue.getEvents request to Last.fm to event information.
	 * Parse response and store the information in the respective objects - Artist, Event, Venue.
	 * @param id - A unique venue id.
	 * @param festivalsOnly - <code>true</code> if only festivals shall be displayed, <code>false</code> if all events shall be displayed.
	 * @return a JsonNode instance containing venue and event information.
	 */
	public JsonNode getVenueEvents(String id, boolean festivalsOnly) {
		
		try{
			// create parameter for Last.fm query to retrieve venue info (especially events)
			...
			// iterate through each event, create object, set variables and add it to venue object
			Venue venue = null;
			boolean isFirst = true;
			if(jsonVenueEvents != null){
				for(JsonNode event : jsonVenueEvents.get("event")){
					
					// for the first element in the list, extract venue information (stored in all events, but only needed once)
					if(isFirst){
						isFirst = false;	// ensures that this if clause is only entered once
						
						// extract venue information and store it in Venue instance
						JsonNode jsonVenue = event.get("venue");
						if(jsonVenue != null){
					...
					// extract event information, store it in Event instance and add it to the Venue instance
					Event e = new Event(event.get("id").toString().replaceAll("\"", ""), event.get("title").toString().replaceAll("\"", ""));
					e.setDate(event.get("startDate").toString().replaceAll("\"", ""));
